...
import (
	...
	"math/rand"
	"strings"
	"time" // Добавлен для инициализации генератора
)
...
func special(charName, charClass string) string {
	switch charClass {
	case "warrior":
		return fmt.Sprintf("%s применил специальное умение `Выносливость %d`", charName, 80+randint(10, 30))
	case "mage":
		return fmt.Sprintf("%s применил специальное умение `Атака %d`", charName, 5+randint(20, 40))
	case "healer":
		return fmt.Sprintf("%s применил специальное умение `Защита %d`", charName, 10+randint(10, 30))
	default:
		return "неизвестный класс персонажа"
	}
}
...
func randint(min, max int) int {
	if min > max {
		min, max = max, min // Защита от некорректных параметров
	}
